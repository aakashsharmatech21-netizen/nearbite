...
export default function BuyerProfileEdit() {
  ...
  useEffect(() => {
    fetch(`${BASE}/api/buyer/profile`, { headers })
      .then(r => r.json())
      .then(d => setForm({ name: d.name || '', phone: d.phone || '', pincode: d.pincode || '' }));
  }, []);

  const handleSave = async () => {
    setSaving(true);
    await fetch(`${BASE}/api/buyer/profile`, { method: 'PATCH', headers, body: JSON.stringify(form) });
    setSaved(true);
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white px-4 py-10">
      <div className="max-w-xl mx-auto">
        <p className="text-xs tracking-widest text-[#555] uppercase mb-1">Buyer Account</p>
